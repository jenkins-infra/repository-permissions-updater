...
class HostingRequestParserTest {
    ...
    @Test
    void basic() {
        Map<String, HostingRequestParser.Field> hostingRequest = HostingRequestParser.convert(
                "### Repository URL\n" +
                        "\n" +
        ...
        assertEquals(
                "dummy-plugin",
                hostingRequest.get("New Repository Name").asString()
        );

        assertEquals(
                "GitHub issues",
                hostingRequest.get("Issue tracker").asString()
        );
